package com.alkaid.dtmp.common.util;
...
public class RedisUtil {
	...
	@Value("${redis.master.max_active}")
	private static int MAX_ACTIVE;

	// 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例，默认值也是8。
	@Value("${redis.master.max_idle}")
	private static int MAX_IDLE;

	// 等待可用连接的最大时间，单位毫秒，默认值为-1，表示永不超时。如果超过等待时间，则直接抛出JedisConnectionException；
	@Value("${redis.master.max_wait}")
	private static int MAX_WAIT;

	// 超时时间
	@Value("${redis.matster.timeout}")
	...
	public static void main(String[] args) {
		RedisUtil.IP = "192.168.43.150";
		RedisUtil.PORT = 6379;
		RedisUtil.PASSWORD = "123456";
		RedisUtil.MAX_ACTIVE = 500;
		RedisUtil.MAX_IDLE = 100;
		RedisUtil.MAX_WAIT = 10000;
		RedisUtil.TIMEOUT = 10000;

		initialPool();

		RedisUtil.set("testKey", "123456-===", 3000);
